...
\end{equation}
This model found $\alpha = \{0.7, 0.86, 1\}$. For the last porous media, a local minimum was not found for $\alpha \in(0,1)$. This model performed slightly worse i.e $\rm{RMSE}_m/\rm{RMSE}_{HP} = \{0.32, 0.45, .48\}$. A sensitivity plot of $\rm{RMSE}_m/\rm{RMSE}_{HP}$ on $\alpha$ can be found in the SI. 

Although we found Eq.(\ref{eq:tau_2}) not to be relevant for all porous media, $\alpha_3$ is needed to find a reliable fit for porous media 3. In addition we found that in this case  the Pearson correlation between Eq.(\ref{eq:tau_1}) and Eq.(\ref{eq:tau_2}) to be $R^2= 0.25$, and can therefore be an indicator that channelization in the form of \ref{eq:tau_2} could still be relevant for pipe-like porous media. 
