...
When we consider a decomposition of the velocity vector  $\mathbf{u} = u_n \mathbf{\hat{p}} + u_s\mathbf{\hat{s}}$ with $\mathbf{\hat{p}} =\mathbf{ \nabla}p/|\mathbf{ \nabla}p|$. For HP flow $\mathbf{u}$ is parallel to the pressure gradient, for which in simple geometries exact solutions can be derived. In the following we will assume that the most important contributions to the dissipation tensor $\nabla_i u_j$ is given by the assumption that $\partial_i u_s \ll \partial_i u_n $ yielding 

\begin{equation}\label{eq:reduced_dissipation_tensor}
\left|\nabla_i u_j\right|^2 \approx  \left|\nabla_s u_n\right|^2 + \left|\nabla_n u_n\right|^2 .
\end{equation}
This assumption has been tested on our numerical experiments described below and are discussed in the SI. The first term in eq.(\ref{eq:reduced_dissipation_tensor}) is much more important since the velocity-velocity correlation length in the longitudinal direction is usually much larger than the average pore size \cite{alim_local_2017}?. When we consider the dissipation of an infinitesimal enclosed volume $dV$ defined by $\mathcal{S}_i,\mathcal{S}_{i+1}$ and their respective areas $A_i, A_{i+1}$, separated by average distance $dx$ we can estimate the first term of Eq.~\ref{eq:reduced_dissipation_tensor} analogous as done by \cite{mortensen_reexamination_2005} with
\begin{equation}
	\left|\nabla_s u_n\right|^2 = \left(\alpha_0+\alpha_1\,\gamma^{-2}\right)\frac{ Q^2}{A^3}~~~ \rm{with}~~~ ,
\end{equation}
...
\begin{equation}
	\left|\nabla_n u_n\right|^2 = \alpha_2  \frac{Q^2}{A^4}\left|\frac{\partial A}{\partial x }\right|^2,
\end{equation}
...
\centering{
\includegraphics[height=5cm]{figures/hydrualic_conductivity_integrated_histogram-1.pdf}
