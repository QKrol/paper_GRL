...
%DIF DEL GRL_PM_flow_local_resistance_v4.tex   Fri Mar  5 03:37:18 2021
%DIF ADD GRL_PM_flow_local_resistance_v5.tex   Wed Jun  2 12:11:18 2021
...
\item We employ direct numerical simulations to measure the local hydraulic conductivity and revisit the \DIFdelbegin \DIFdel{classical model of pore resistance based on }\DIFdelend Hagen-Poiseuille law. 
\item \DIFdelbegin \DIFdel{While Hagen-Poiseuille systematically underestimates low resistances, a new }\DIFdelend \DIFaddbegin \DIFadd{We show that a }\DIFaddend circularity-based model performs significantly better \DIFdelbegin \DIFdel{by a reduction of the root-mean-squared-relative error from (on average) $45\%$ to $15\%$}\DIFdelend \DIFaddbegin \DIFadd{than the Hagen-Poiseuille model, reducing the averaged error by a third}\DIFaddend .
\end{keypoints}


\begin{abstract}
We examine the validity of the commonly used Hagen-Poiseuille \DIFdelbegin \DIFdel{(HP) }\DIFdelend model of local resistance of porous media \DIFaddbegin \DIFadd{using direct numerical simulations}\DIFaddend . We provide theoretical arguments that highlight possible limitations of this model and formulate a new constitutive model that is based on \DIFdelbegin \DIFdel{local pore geometry}\DIFdelend \DIFaddbegin \DIFadd{the circularity of iso-pressure surfaces}\DIFaddend . We compare the performance of both models on three different three-dimensional artificial porous media\DIFdelbegin \DIFdel{obtained by using thresholding on Gaussian random fields. Direct numerical simulations are used to solve the pressure and velocity field of the laminar flow in the pore-spaces of these geometries. We introduce a new definition for pores based on iso-pressure surfaces and topology changes that enables us to measure the local hydraulic resistance. We }\DIFdelend \DIFaddbegin \DIFadd{. We }\DIFaddend show that the \DIFdelbegin \DIFdel{HP model overestimates the local hydraulic resistance by up to one order of magnitude. Our new model demonstrates that the circularity of iso-pressure surfaces is key to predict the local hydraulic resistances of pores in heterogeneous porous media. This model }\DIFdelend \DIFaddbegin \DIFadd{new model }\DIFaddend improves the root-mean-squared-relative error from $59\%$, $48\%$ and $32\%$ for the HP model to $12\%$, $14\%$ and $18\%$ for the three porous media respectively. We anticipate that our approach may find broad application in \DIFdelbegin \DIFdel{pore network models }\DIFdelend \DIFaddbegin \DIFadd{network models of porous media }\DIFaddend that are typically build from 3D images \DIFdelbegin \DIFdel{of porous material }\DIFdelend with intricate pore geometries.
\end{abstract}
...
\DIFaddbegin \section*{\DIFadd{Index terms and keywords}}
\DIFadd{5114 Permeability and porosity 5139 Transport properties, 0550 (Model verification and validation)
}
...
\end{figure}
Here we introduce the notion of disconnected iso-pressure surface $\mathcal{S}_i(p)$ for a given pressure value $p$. Iso-pressure surfaces are usually disconnected because they exist in the fluid domain only and are thus interrupted by the solid phase of the media. The first term of Eq.~(\ref{eq:stokes_dissipation}), the boundary term, will be less significant when the total volume $V$ is enlarged by increasing $\delta p$. Furthermore, when we have saturated conditions, the complete pore space can be compartmentalized in a network of enclosed volumes $V_i(p_i, p_i+\delta p_i )$, which we will later call pores. \DIFdelbegin \DIFdel{For all the pore spaces in the network the boundary term only exist virtually, since the same boundary term is subtracted for the pores }\DIFdelend \DIFaddbegin \DIFadd{We have assessed the relevance of the boundary term }\DIFaddend to \DIFdelbegin \DIFdel{which they are physically connected. Therefore }\DIFdelend \DIFaddbegin \DIFadd{$Q\Delta p$ for ten pores in the SI. We found that they contribute generally below $5\%$ for the shortest available pores to below $1\%$ for pores five times as long. Therefore it is reasonable to estimate }\DIFaddend Eq.~(\ref{eq:stokes_dissipation}) \DIFdelbegin \DIFdel{intrinsically only evaluates
}\DIFdelend \DIFaddbegin \DIFadd{by
}\DIFaddend \begin{equation}
Q \Delta p\DIFdelbegin \DIFdel{=}\DIFdelend \DIFaddbegin \DIFadd{\approx}\DIFaddend \mu\int (\nabla \otimes \mathbf{u})^2 dV.\label{eq:pore_based_energy_dissipation}
\end{equation}

In the following we apply a decomposition of the velocity vector $\mathbf{u} = u_p \mathbf{\hat{p}} + u_r \mathbf{\hat{r}}$ with $\mathbf{\hat{p}} =\mathbf{ \nabla}p/|\mathbf{ \nabla}p|$ (longitudinal direction in respect to the flow) and $\hat{\mathbf{p}}$ perpendicular to $\hat{\mathbf{r}}$ (transversal direction in respect to the flow). We assume that the most important contributions to the viscous dissipation tensor $\nabla_i u_j$ \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend given by $\nabla_i u_p$ i.e. $\mathbf{\nabla}_i u_r \ll \mathbf{\nabla}_i u_p $. \DIFdelbegin \DIFdel{This }\DIFdelend \DIFaddbegin \DIFadd{Also this }\DIFaddend assumption has been verified in the supplementary information (SI) for the porous media that we have used below and leads to 
\begin{equation}
\left|\nabla_i u_j\right|^2 \approx  \left|\nabla_r u_p\right|^2 + \left|\nabla_p u_p\right|^2 .\label{eq:reduced_dissipation_tensor}
\end{equation}
Here the first term is expected to be more important for gradually varying pore geometries since gradients in the velocity in the longitudinal direction are usually much lower than in transverse direction. Equations (\ref{eq:stokes_local})-(\ref{eq:reduced_dissipation_tensor}) are valid for arbitrary volumes $V$. When we consider viscous dissipation \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend an \textit{infinitesimal} \DIFdelbegin \DIFdel{enclosed }\DIFdelend volume $dV$ \DIFdelbegin \DIFdel{defined }\DIFdelend \DIFaddbegin \DIFadd{enclosed }\DIFaddend by $\mathcal{S}(p),\mathcal{S}(p+\delta p)$ \DIFdelbegin \DIFdel{and their }\DIFdelend \DIFaddbegin \DIFadd{(with }\DIFaddend respective areas $A(p), A(p+\delta p)$\DIFaddbegin \DIFadd{)}\DIFaddend , separated by average distance $dx$ defined by $dV = A(p)dx$, we can estimate (analogous to \citeA{mortensen_reexamination_2005}) the average value of the first term of Eq.~(\ref{eq:reduced_dissipation_tensor}) by
\begin{equation}
	\left|\nabla_r u_p\right|^2 =8\pi \left(\alpha_0+\alpha_1\,\mathcal{C}\right)\frac{ Q^2}{A^3}~~~ ,\label{eq:tau_1}
\end{equation}
with circularity parameter $\mathcal{C} = \mathcal{L}^2/4\pi A(p)$ with perimeter $\mathcal{L} = \int_{\partial \mathcal{S}(p)}dl$. The circularity parameter is related to the compactness factor $C = \mathcal{C}/4\pi$ in \cite{mortensen_reexamination_2005}, and for HP flow \DIFaddbegin \DIFadd{it is }\DIFaddend equal to one. The coefficients $\alpha_0$ and $\alpha_1$ can be calculated (in first order of circularity) analytically or numerically for simple shapes of the iso-pressure surfaces, such as squares, triangles, or a perturbation of a sphere by spherical harmonics \cite{mortensen_reexamination_2005}. For heterogeneous media the class of shapes are generally unknown and not symmetric\DIFaddbegin \DIFadd{, and therefore $\alpha_0$ and $\alpha_1$ are expected to be intrinsically dependent on the pore geometry and therefore to change from pore to pore}\DIFaddend .  
...
\end{equation}
with \DIFdelbegin \DIFdel{proportonality }\DIFdelend \DIFaddbegin \DIFadd{proportionality }\DIFaddend factor $\alpha_2$. \DIFaddbegin \DIFadd{Again $\alpha_2$ is intrinsically dependent on pore geometry and changes from pore to pore. }\DIFaddend We combine the two expressions Eq.~(\ref{eq:tau_1}) and Eq.~(\ref{eq:tau_2}) with Eq.~(\ref{eq:pore_based_energy_dissipation}) into
\begin{equation}
	\frac{dp}{dx} = 8\pi \mu \frac{Q(p)}{A(p)^2} f\left(\alpha_i,\mathcal{S}(p) \right)\DIFdelbegin \DIFdel{= 8 \pi \mu\frac{Q}{A^2}}%DIFDELCMD < \left(%%%
\DIFdel{\alpha_0+\alpha_1\mathcal{C} + \alpha_2 \frac{1}{A}}%DIFDELCMD < \left|%%%
\DIFdel{\frac{d A}{d x}}%DIFDELCMD < \right|%%%
\DIFdel{^2}%DIFDELCMD < \right)%%%
\DIFdel{\,}\DIFdelend \DIFaddbegin \DIFadd{,}\DIFaddend \label{eq:infi_dp}
\DIFdelbegin %DIFDELCMD < \end{equation}
%DIFDELCMD < %%%
\DIFdel{This expression }\DIFdelend \DIFaddbegin }
\DIFadd{with 
}\begin{equation}
	 \DIFadd{f\left(\alpha_i,\mathcal{S}(p)\right) = \alpha_0+\alpha_1\mathcal{C} + \alpha_2 \frac{1}{A}\left|\frac{d A}{d x}\right|^2.\label{eq:f1}
}\end{equation} 	
\DIFadd{This parametrization }\DIFaddend is consistent with the Hagen-Poiseuille equation for pipe geometries $f(\alpha_i)\rightarrow 1$, for which the local infinitesimal pressure gradient is given by 
\begin{equation}
	\frac{d p}{d x} = 8 \pi \mu\frac{Q}{A^2}\label{eq:HP}.
\end{equation} 
As long as the total flux $Q$ remains constant and $dV = Adx$ remains valid, Eq.~(\ref{eq:infi_dp}) can be integrated over $\Delta p$. We define a pore by the integrated \DIFdelbegin \DIFdel{enclosed }\DIFdelend volume, bound by iso-pressure surfaces $\mathcal{S}_i(p)$, $\mathcal{S}_j(p+\Delta p)$ and the porous media boundary. The hydraulic resistance of a pore is then given by $\mathcal{R}= \frac{\Delta p}{Q}$. The right-hand side of Eq.~(\ref{eq:pore_based_energy_dissipation}) gives us therefore a \DIFaddbegin \DIFadd{statistical }\DIFaddend model for the hydraulic resistance \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD <  %%%
\DIFdel{From this we find a model for the total hydraulic resistance of a pore
}\DIFdelend \DIFaddbegin \DIFadd{$\mathcal{R}_m$ given by
}\DIFaddend \begin{equation}
	\mathcal{R}_m = \DIFaddbegin \DIFadd{8\pi\mu}\DIFaddend \int^{L_{\rm{eff}}}_{0}\frac{1}{A^2}f\left(\alpha_i,\mathcal{S}(p)\right)\,dx\label{eq:R_model},
\end{equation}
...
\caption{Top left: Measured infinitesimal pressure gradient versus the infinitesimal pressure drop of a Hagen-Poiseuille model, i.e. $f(\alpha)\rightarrow 1$, for all three porous media combined. The marker size is scaled with square root of the averaged area $\sqrt{\overline{A}}$. Top right: Measured versus Modeled infinitesimal pressure gradient with $f(\alpha)$ fitted to Eq.~(\ref{eq:infi_dp}) for each porous media separately. Bottom: The marker size is scaled by $N\sqrt{\overline{A}}$, with N the number of consecutive patches. \DIFdelbeginFL \DIFdelFL{Left}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Bottom left}\DIFaddendFL : Integrated pressure drop vs $\mathcal{R}_{HP}$ times the total Flux $Q$\DIFdelbeginFL \DIFdelFL{Right}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{. Bottom right}\DIFaddendFL : Integrated pressure drop divided by the total Flux $Q$ vs $\mathcal{R}_m$ (Eq.~(\ref{eq:R_model})). }
\label{fig:local_and_integrated}
\end{figure}

\DIFdelbegin \DIFdel{Since we obtain a range of complex shapes for $\mathcal{S}$ the obtained values for $\alpha_i$ are }\DIFdelend \DIFaddbegin \DIFadd{We visually observe that the circumferences of the iso-pressure surfaces are not smooth and lead to an overestimation of $\mathcal{C}_i(p)$. In the SI we have evaluated this error to be a factor $\epsilon = 1.15,~1.11,~1.08$ for the three porous media respectively. The origin of this error is the grid refinement near the boundary. Given that this is }\DIFaddend expected to be \DIFdelbegin \DIFdel{an average. Therefore }\DIFdelend \DIFaddbegin \DIFadd{uniform throughout the porous media, we expect the error to be similar for all $\mathcal{C}_i(p)$. Given this observation and that the last term in Eq.~\ref{eq:f1} is insignificant, }\DIFaddend it is interesting to test the robustness of the linearity in $\mathcal{C}$ and the consistency of the function $f \rightarrow 1$ when $\mathcal{C}\rightarrow 1$. We introduce an alternative function $g$ to $f$ with two fit parameters $\alpha$ and $\beta$,
\begin{equation}
...
\section{Conclusion}
We have proposed a new iso-pressure surface based definition for individual pores in heterogeneous porous media with the aim of measuring and modeling the local hydraulic resistance which can potentially be used in a pore-network model. This new definition uses the constant flux as constraint on the length of the pore. The definition of the pores allows us to estimate the local hydraulic resistance in terms of the viscous dissipation tensor. This can be modeled by Eq.~(\ref{eq:R_model})\DIFdelbegin \DIFdel{containing the circularity of the iso-pressure surfaces. }\DIFdelend \DIFaddbegin \DIFadd{, with $\alpha_2=0$ resulting in  
}\begin{equation}
 	\DIFadd{\mathcal{R} = 8\pi \mu\int^{L_{\rm{eff}}}_{0}\frac{1}{A^2}\left(\alpha_0+\alpha_1\mathcal{C}\right)\,dx\label{eq:R_model}.
 }\end{equation} 
\DIFaddend This model significantly improves the Hagen-Poiseuille model for heterogeneous media. 

\DIFdelbegin %DIFDELCMD < \acknowledgments{The authors acknowledge SNSF grant nr.172916 and valuable discussions with H. L\"owe and M. Morales.}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \acknowledgments{The authors acknowledge support through SNSF grant nr.172916 and valuable discussions with H. L\"owe and V. L. Morales.}
\DIFaddend 
