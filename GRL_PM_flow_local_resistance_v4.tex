...
\end{equation} 
As long as the total flux $Q$ remains constant and $dV = Adx$ remains valid, surfaces Eq.~(\ref{eq:infi_dp}) can be integrated. We therefore use this constraint to define the beginning and the end of a single pore. The average pressure gradient along a pore is then defined by the total hydaulic resitance $\mathcal{R}$ of a pore and therefore given by
\begin{equation}
	\frac{\Delta p}{L_{\rm{eff}}} = \mathcal{R} =8\pi \mu  Q \frac{1}{L_{\rm{eff}}}\int^{L_{\rm{eff}}}_{0}\frac{1}{A^2}f\left(\alpha_i,\mathcal{S}(p)\right)\,dx,
\end{equation}
with $L_{\rm{eff}}$ the total length of the pore. From this we find a model for the total hydraulic resistance of a pore
\begin{equation}
