...
%DIF DEL GRL_PM_flow_local_resistance_v3.tex   Mon Jan 25 16:29:34 2021
%DIF ADD GRL_PM_flow_local_resistance_v4.tex   Sun Feb 14 16:58:48 2021
...
\DIFdelbegin %DIFDELCMD < \authors{Quirine Krol \affil{1}, Itzhak Fouxon \affil{1,2}, Pascal Corso \affil{1}, Markus Holzner \affil{1,2,3,4}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \authors{Quirine Krol\affil{1}, Itzhak Fouxon \affil{1,2}, Pascal Corso \affil{1}, Markus Holzner \affil{1,3,4}}
\DIFaddend 
...
\DIFdelbeginFL %DIFDELCMD < \includegraphics[height=5cm]{figures/example_pore_v2.eps}%%%
\DIFdelFL{~~~~
}%DIFDELCMD < \includegraphics[height=5cm]{figures/merging_pores_v2.eps}
%DIFDELCMD < %%%
...
For the second, longitudinal term, we can assume that the total flux $Q$ remains constant for $p\rightarrow p+dp$, and the change of the velocity in longitudinal direction is caused by a change in cross-sectional area $A(p)\rightarrow A(p+\delta p)$\DIFaddbegin \DIFadd{. We estimate $u_p$ by the total flux $Q/A$}\DIFaddend , i.e. 
\begin{equation}
	\left|\nabla_p u_p\right|^2 =  8\pi  \DIFdelbegin \DIFdel{\mu }\DIFdelend \alpha_2\frac{Q^2}{A^4}\left|\frac{dA}{dx }\right|^2,\label{eq:tau_2}
\end{equation}
with \DIFdelbegin \DIFdel{unknown }\DIFdelend \DIFaddbegin \DIFadd{proportonality factor }\DIFaddend $\alpha_2$. \DIFdelbegin \DIFdel{For the case where the infinitesimal pressure difference $dp$, no topological changes (no junctions) of the iso-pressure surfaces take place, we can write $dV = A dx$. }\DIFdelend We combine the two expressions Eq.~(\ref{eq:tau_1}) and Eq.~(\ref{eq:tau_2}) with Eq.~(\ref{eq:pore_based_energy_dissipation}) into
\DIFdelbegin %DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdelend \begin{equation}
	\frac{d p}{d x} = 8 \pi \mu\frac{Q}{A^2}\DIFdelbegin \DIFdel{, }\DIFdelend \label{eq:HP}.
\end{equation} 
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend As long as \DIFdelbegin \DIFdel{there are no junctions or topological changes to the consecutive iso-pressure surfaces }\DIFdelend \DIFaddbegin \DIFadd{the total flux $Q$ remains constant and $dV = Adx$ remains valid, }\DIFaddend Eq.~(\ref{eq:infi_dp}) can be integrated \DIFdelbegin \DIFdel{. This restriction is therefore used to define a single pore . The average pressure gradient along }\DIFdelend \DIFaddbegin \DIFadd{over $\Delta p$. We define a pore by the integrated enclosed volume, bound by iso-pressure surfaces $\mathcal{S}_i(p)$, $\mathcal{S}_j(p+\Delta p)$ and the porous media boundary. The hydraulic resistance of }\DIFaddend a pore is then given by \DIFaddbegin \DIFadd{$\mathcal{R}= \frac{\Delta p}{Q}$. The right-hand side of Eq.~(\ref{eq:pore_based_energy_dissipation}) gives us therefore a model for the hydraulic resistance 
}\DIFaddend 

 \DIFdelbegin \begin{displaymath}
	\DIFdel{\frac{\Delta p}{L_{\rm{eff}}} =  8\pi \mu  Q \frac{1}{L_{\rm{eff}}}\int^{L_{\rm{eff}}}_{0}\frac{1}{A^2}f\left(\alpha_i,\mathcal{S}(p)\right)\,dx,
}\end{displaymath}
%DIFAUXCMD
\DIFdel{with $L_{\rm{eff}}$ the total length of the pore.}\DIFdelend From this we find a \DIFdelbegin \DIFdel{modeled expression }\DIFdelend \DIFaddbegin \DIFadd{model }\DIFaddend for the total hydraulic resistance of a pore
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \begin{equation}
	\mathcal{R}_m = \int^{L_{\rm{eff}}}_{0}\frac{1}{A^2}f\left(\alpha_i,\mathcal{S}(p)\right)\,dx\label{eq:R_model}\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{,
}\DIFaddend \end{equation}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{with $L_{\rm{eff}}= \int\,dx $ the total effective length of the pore. }\DIFaddend When the geometry of a media is given by a long pipe, the hydraulic resistance is given by the Hagen-Poiseuille \DIFdelbegin \DIFdel{model $R_m = R_{\rm{HP}}$, meaning that $f(\alpha_i)\rightarrow 1$ and will therefore only depend on the cross-section }\DIFdelend \DIFaddbegin \DIFadd{(HP) model $\mathcal{R}_{\rm{HP}}$, given by $\mathcal{R}_m(f\rightarrow 1)$. The HP model therefore only depends on the cross-sectional }\DIFaddend area $A(p)$.
...
To generate heterogeneous porous media \DIFdelbegin \DIFdel{that will form the basis for the direct numerical simulations, we have used }\DIFdelend \DIFaddbegin \DIFadd{we make use of the }\DIFaddend Gaussian Random Fields (GRF)\DIFdelbegin \DIFdel{to generate 3 porous media }\DIFdelend \DIFaddbegin \DIFadd{, which are increasingly used to represent realistic porous media \mbox{%DIFAUXCMD
\cite{liu_advances_2019}}%DIFAUXCMD
. We used a fast fourier transform and a spectral density function to generate GRF scalar functions }\DIFaddend \cite{teubner_level_1991,hyman_heterogeneities_2012,siena_relationship_2014}. A threshold \DIFdelbegin \DIFdel{of the field function of the GRF }\DIFdelend \DIFaddbegin \DIFadd{on the GRF function }\DIFaddend is used to define the porous media-fluid interface $\Gamma$ with porosities $0.68, 0.34$ and $0.17$ respectively. \DIFdelbegin \DIFdel{In the SI we have listed a few }\DIFdelend \DIFaddbegin \DIFadd{For details on the GRF functions and }\DIFaddend geometrical parameters such as average pore size and surface roughness \DIFaddbegin \DIFadd{are given in the SI}\DIFaddend . These porous media are used as input for direct numerical simulations (DNS, OpenFOAM v. 4.1, \citeA{weller_tensorial_1998}), that solve the \DIFdelbegin \DIFdel{steady state Navier-Stokes equations }\DIFdelend \DIFaddbegin \DIFadd{Stokes equations (Eq.~\ref{eq:stokes_local}) }\DIFaddend in the pore space. The boundary conditions are defined at the inlet $p_1$ and outlet $p_2$ and a no-slip condition for the porous media-fluid interface. A visualization of the three porous media is shown in Fig.~\ref{fig:DNS}. Next, a chain of visualization toolkit (VTK) based image analysis techniques \cite{schroeder_visualization_2006,hernderson_paraview_2007} is employed to extract iso-pressure surfaces $\mathcal{S}(p)$ and enumerate the disconnected areas identifying as an iso-pressure patch $\mathcal{S}_i(p)$. This patch is part of a pore and has a surface area $A_i(p)$, circularity $\mathcal{C}_i(p)$, \DIFdelbegin \DIFdel{average location }\DIFdelend \DIFaddbegin \DIFadd{center of `mass' of iso-pressure surface }\DIFaddend $\mathbf{X}_i(p)$ and \DIFdelbegin \DIFdel{average }\DIFdelend \DIFaddbegin \DIFadd{total }\DIFaddend flux $Q_i(p)$. For each $\mathcal{S}_i(p)$ we identify its closest neighbor $\mathcal{S}_j(p+\delta p)$. This neighboring iso-pressure patch (building up a pore) is found by calculating the distance function $f_d(\mathbf{x},\mathcal{S})$, between \DIFaddbegin \DIFadd{any given }\DIFaddend point $\mathbf{x}\in \mathcal{S}_i(p)$ and all iso-pressure patches $\mathcal{S}_k(p+\delta p)$. This distance function is defined by
\DIFdelbegin %DIFDELCMD < 
...
\end{equation}
The closest neighbor $\mathcal{S}_j(p+\delta p)$ is found by the minimum value of $d_{i,j} = \rm{min}\left\{ d_{i,k}\right\}$. When $\delta p$ is chosen sufficiently small the enclosed volume can be estimated by $V_i(p,\delta p)=A_id x_i \approx A_i d_{i,j}$. We use forward integration of consecutive patches until merging or splitting takes place. \DIFdelbegin \DIFdel{The distances between averaged locations }\DIFdelend \DIFaddbegin \DIFadd{This is translated into constraints on flux conservation and an upper bound for $d_{i,j}$. We noticed however that the distances between }\DIFaddend $\mathbf{X}_i(p)$ and $\mathbf{X}_j(p+\delta p)$ of two consecutive pores are \DIFaddbegin \DIFadd{more }\DIFaddend sensitive to topology changes\DIFdelbegin \DIFdel{. A constraint on these distances and flux conservation is used to end forward integration}\DIFdelend \DIFaddbegin \DIFadd{, and are therefore used instead}\DIFaddend . The precise values for these constraints can be found in the SI. A demonstration of the correct identification of pores by forward integration is shown in Fig.~\ref{fig:isop_surfaces} (Right), showing a merging of two pores. \DIFaddbegin \DIFadd{Although the proposed definition of individual pores deals naturally with junctions, a straight forward pore-network implementation is still missing. This is partly due to the exclusion of iso-pressure patches that are singular, and have no neighboring patches due to rapid changing topologies. The ratios of excluded surface area patches are given by $18 \%, 26 \%$ and $2\%$, which, at least for the first two porous media, prevents a continuous integration of the network-topology.
}\DIFaddend 

The main challenge with the data format of the OpenFoam simulations is that it is unstructured, and the meshing is refined towards the boundary of the porous media $\Gamma$. Although this ensures that the geometry is accurately described and that the simulation \DIFdelbegin \DIFdel{convergences}\DIFdelend \DIFaddbegin \DIFadd{converges}\DIFaddend , it also causes challenges in the extraction of $\mathcal{S}(p)$ by using a VTK contour filter. Since it is based on a threshold on $p$ it breaks up the mesh close to $\Gamma$ into many disconnected noisy area patches. These are removed by applying a filter on the area size of the patches, resulting in a reduction of total surface area of \DIFdelbegin \DIFdel{maximal $3\%$}\DIFdelend \DIFaddbegin \DIFadd{maximally $2\%$}\DIFaddend . Extracting circularity $\mathcal{C}_i(p)$ is achieved by applying a contour filter on a low value for the magnitude of the velocity $u_{\rm{min}} = 10^{-9}~\rm{m/s}$ (4 orders smaller than the averaged values)\DIFaddbegin \DIFadd{, obtaining the circumference $\mathcal{L}_i(p)$ of the area patch close to the porous media boundary}\DIFaddend . We observe \DIFdelbegin \DIFdel{however }\DIFdelend that these contours are locally irregular\DIFdelbegin \DIFdel{and therefore will very likely lead to an overestimation of $\mathcal{C}$, which will influence the fitting value of $\alpha_1$. 
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{. Choosing a higher value for $u_{\rm{min}}$ did not lead to a reduction in the circumference but rather an increase due to separation of the area patch in smaller ones. The irregularity in $\mathcal{C}_i(p)$, is likely due to irregularities in the mesh, and therefore similar for all area patches. 
...
The result of the DNS for the three porous media is shown in Fig.~\ref{fig:DNS}. The Reynolds numbers are calculated by $\rm{Re} \sim \ell_p q/\nu$, with $q = Q/\overline{A}$ and the average pore size defined by the volume to total porous media interface ratio $\ell_p = 4 \phi V/|\Gamma|$. For all porous media $\rm{Re}$ is smaller than $10^{-2}$. In Fig.~\ref{fig:DNS}, bottom, the results of the infinitesimal pressure gradients versus the HP model (Eq.~(\ref{eq:HP})) are shown. We observe that the HP model underestimates the pressure gradient \DIFaddbegin \DIFadd{by }\DIFaddend up to two orders of magnitude for the first porous media, to a relative good estimate for the third. We notice that $\mathcal{C}$ is the lowest for smallest pores, indicating that smaller iso-pressure surfaces are more circular than larger more complex shaped iso-pressure surfaces. \DIFdelbegin \DIFdel{We have plotted the joined datasets of the three porous media and observe that (Fig.~\ref{fig:local_and_integrated}, top left) there is no visual }\DIFdelend \DIFaddbegin \DIFadd{Besides the different ranges covered we see no visual visual }\DIFaddend distinction between the three porous media, i.e. their data overlap and behave uniformly with respect to $\mathcal{C}$ and size $A$\DIFdelbegin \DIFdel{.
 }\DIFdelend \DIFaddbegin \DIFadd{, see Fig.~\ref{fig:local_and_integrated}, top left.
 }\DIFaddend 

For each porous media Eq.~(\ref{eq:infi_dp}) has been fitted by minimizing the least-squared error independently to obtain estimates for $\alpha_i$. The contribution of the term $\alpha_2$ is insignificant for all three porous media and is reported in the SI. A simple fit, excluding $\alpha_2$ resulted in three values for $\alpha_0 = 0.48, 0.52, 0.19$ and $\alpha_1 = 0.90, 0.87, 1.16$ for the corresponding porous media respectively. The result of the fitting is shown in Fig.~\DIFdelbegin \DIFdel{(}\DIFdelend \ref{fig:local_and_integrated}, top right\DIFdelbegin \DIFdel{)}\DIFdelend . Using Eq.~(\ref{eq:infi_dp}) with the fitted values for $\alpha_0$ and $\alpha_1$ ($\alpha_2 = 0$) we obtain a new model for local pore resistances $\mathcal{R}_m$, Eq.~(\ref{eq:R_model}), which performs much better than the HP model. The result is shown in Fig.~\ref{fig:local_and_integrated}, bottom right. 

The Pearson correlation coefficients \DIFaddbegin \DIFadd{$R^2$ }\DIFaddend for all models of the resistances are higher than $0.88$. For the HP model the values for $R^2$ are given by $0.91, 0.88$ and $0.99$. The coefficients $R^2$ of $\mathcal{R}_m$ are given by $0.97, 0.95$ and $0.99$. The high values are caused by the large domain size spanning several orders of magnitude. The deviations of $\mathcal{R}_{\rm{HP}}$ with the measured values $\Delta p/Q$ are not uniform across the scales and  therefore $R^2$ is not a reliable parameter when it comes to expressing the improvement over the HP model \cite{wilcox_comparing_2009}. We therefore calculated the reduction of the root-mean-squared-relative error (RMSRE) from $\mathcal{R}_{\rm{HP}}$ to $\mathcal{R}_{\rm{m}}$. For the first porous media we found a reduction in the RMSRE from $59\%$ to $12\%$, for the second from $46\%$ to $13\%$ and for the last from $31\%$ to $15\%$.  
...
\DIFdelbegin %DIFDELCMD < \acknowledgments{Q. Krol acknowledges dr. H.L\"owe for valuable discussions and  M. Holzner acknowledges grant ... }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \acknowledgments{The authors acknowledge SNSF grant nr.172916 and valuable discussions with H. L\"owe and M. Morales.}
\DIFaddend 
